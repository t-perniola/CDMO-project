...
def read_data_file(file_path):
    ...
    package_weights = list(map(int, lines[3].strip().split()))
    
    distance_matrix = [list(map(int, line.strip().split())) for line in lines[4:-2]]
    lower_bound = int(lines[-2].strip())
    upper_bound = int(lines[-1].strip())
