...
def json_fun(instance_number, optimal, obj, paths, time_taken, symm_break, search_strategy):
    ...
    time_taken_int = int(floor(time_taken))
    
    json_dict = {
        "time": TIME_LIMIT if time_taken_int > TIME_LIMIT else time_taken_int,
        "optimal": optimal,
...
def SAT(instance_num, sb_bool=False, search_method="branch_and_bound"):
    ...
    if solution:
        ...
        json_fun(instance_num, solution[search_method]["optimal"], solution[search_method]['obj'], solution[search_method]['sol'], time_taken, sb_bool, search_method)
    else:
        print("- Objective value (max dist): No feasible solution found (UNSAT).\n")
        json_fun(instance_num, False, None, None, time_taken, sb_bool, search_method)
