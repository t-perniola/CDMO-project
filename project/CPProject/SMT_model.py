...
for c in COURIERS:
    for j in range(max_nodes): # 'select(path, idx)' retrieves the value stored in the array 'path' at index 'idx'
        optimizer.add(And(Select(path, c * max_nodes + j) >= 0, Select(path, c * max_nodes + j) <= n+1))
